...
export default function LoginPage() {
  ...
  const [availableCampuses, setAvailableCampuses] = useState<Campus[]>(campuses);

  React.useEffect(() => {
    const loadCampuses = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/campuses`);
        if (!response.ok) {
